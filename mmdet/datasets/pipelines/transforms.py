...
class Corrupt:
    ...
    def __init__(self, corruption, severity=1, P=0.5):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if x <= self.P:
                if self.corruption == 'snow':
                    severity = random.randint(0,2)+1
                elif self.corruption == 'fog ':
                    severity = random.randint(0,2)+3
                ...
                # finish = cv2.imwrite (f'/home/data/jmh/ECCV2022_OOD/object-detection/img_show/Albu_snow/Albu_snow_{a}.jpg',results[key])
                # print(finish)
        # results['img'] = corrupt(
        #     results['img'].astype(np.uint8),
        #     corruption_name=self.corruption,
...
class Albu:
    ...
    def __call__(self, results):
        ...
        # for key in results.get('img_fields', ['img']):
        #     a = random.randint(0,100)
        #     finish = cv2.imwrite (f'/home/data/jmh/ECCV2022_OOD/object-detection/img_show/Albu_rain/Albu_rain_{a}.jpg',results[key])
        #     print(finish)
...
@PIPELINES.register_module()
class Occlusion:
    def __init__(self, screen_imgDirPath, p = 0.5):
        self.screen_imgDirPath = screen_imgDirPath
        self.imgs = []
        self.p = p
        dirs = os.listdir(self.screen_imgDirPath)
        for dir in dirs:
            imgPath = self.screen_imgDirPath+'/'+dir
            img = cv2.imread(imgPath)
            self.imgs.append(img)

    def __call__(self, results):

        #将指定的screen_img图像左右随机的加入到raw_img图像上，返回加入后的图像
        def add_img(raw_img,screen_img):
            #图像resize
            h1, w1 = raw_img.shape[0], raw_img.shape[1]
            h2, w2 = screen_img.shape[0], screen_img.shape[1]
            screen_img = cv2.resize(screen_img,(min(int(h1/h2*w2),w1),h1))
            h2, w2 = screen_img.shape[0], screen_img.shape[1]

            #padding top, bottom, left, right
            padding_top = int(h1*0.5-h2*0.5)
            padding_bottom = h1-h2-int(h1*0.5-h2*0.5)
            #控制图片左右的距离
            distance = np.random.rand()
            padding_left = int((w1 - w2) * distance) #int(w1*0.5-w2*0.5)
            padding_right = w1 - w2 -padding_left #w1-w2-int(w1*0.5-w2*0.5)
            screen_img = cv2.copyMakeBorder(screen_img, padding_top, padding_bottom, padding_left, padding_right, cv2.BORDER_CONSTANT, value=(0,0,0))

            #图像叠加
            #screen_img_new = screen_img*((screen_img!=255).astype(int))
            raw_img_2 = raw_img * ((screen_img == 0).astype(int))
            occlusion_img = raw_img_2 + screen_img
            return occlusion_img
        
        def add_img2(raw_img,screen_img):
            h1, w1 = raw_img.shape[0], raw_img.shape[1]
            h2, w2 = screen_img.shape[0], screen_img.shape[1]
            #宽
            if w2 / h2 > w1 / h1 * 0.8 or w2 / h2 > 1.2:
                wRatio = np.random.randint(700,1100)*0.001
                screen_img = cv2.resize(  
                                screen_img,((int(w1 * wRatio),
                                        int(w1 * wRatio * h2 / w2 * np.random.randint(850, 1150) * 0.001))))
            else:
                hRatio = np.random.randint(800,1200)*0.001
                screen_img = cv2.resize ( 
                                screen_img,(int((h1 * hRatio) / h2 * w2 * np.random.randint(850, 1150) * 0.001),
                                        int(h1*hRatio)))
            #cv2.imwrite('/home/data/qxh/code/tools/test/occlusion_img_'+str(ans)+'.jpg',screen_img)
            screen_img = cv2.copyMakeBorder(screen_img, int(screen_img.shape[0] * 0.3), int(screen_img.shape[0] * 0.3), int(screen_img.shape[1] * 0.3), int(screen_img.shape[1] * 0.3), cv2.BORDER_CONSTANT, value=(0,0,0))
            h2, w2 = screen_img.shape[0], screen_img.shape[1]

            # 加入随机旋转，范围为（-30°，30°）
            theta = np.random.randint(-20 , 21)  # 顺时针旋转角度，单位为角度
            x0, y0 = w2//2, h2//2  # 以图像中心作为旋转中心
            MAR = cv2.getRotationMatrix2D((x0,y0), theta, 1.0)
            screen_img = cv2.warpAffine(screen_img, MAR, (w2, h2))  # 旋转变换，默认为黑色填充
            #cv2.imwrite('/home/data/qxh/code/tools/test/screen_img_warp.png',screen_img)

            #选取原始图像点，进行叠加
            if w2/h2 > w1/h1*0.8 or w2 / h2 > 1.2:
                centerW, centerH = np.random.randint(int(0), int(w1)), np.random.randint(int(0), int(h1))
            else:
                centerW, centerH = np.random.randint(int(w1*0.25), int(w1 * 0.75)), np.random.randint(int(h1*0.25), int(h1 * 0.75))
            centerw, centerh = w2 // 2, h2 // 2
            # #padding top, bottom, left, right
            padding_top = max(centerH- h2 // 2, 0)+1
            centerh += padding_top
            padding_bottom = max(h1 - padding_top, 0)+1
            padding_left = max(centerW- w2 // 2, 0)+1
            centerw += padding_left
            padding_right = max(w1 - padding_left, 0)+1
            screen_img = cv2.copyMakeBorder(screen_img, padding_top, padding_bottom, padding_left, padding_right, cv2.BORDER_CONSTANT, value=(0,0,0))
            h2, w2 = screen_img.shape[0], screen_img.shape[1]
            # 高 宽
            screen_img = screen_img[centerh - centerH:centerh - centerH+h1, centerw - centerW:centerw - centerW+w1]
        ...
        for key in results.get('img_fields', ['img']):
            ...
            #从候选集中随机选取一张图片
            randomPos = int(np.random.rand()*len(self.imgs))
            #add操作，返回处理之后的图片
            pro = random.random()
            if pro <= self.p:
                results[key] = add_img2(results[key], self.imgs[randomPos]).astype("float")
            #cv2.imwrite ('/home/data/jmh/ECCV2022_OOD/object-detection/mmdetection2/_my_configs/occlusion_exp/exp.jpg', results[key])
            
        return results
        
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(screen_imgDirPath={self.screen_imgDirPath})'
        return repr_str
